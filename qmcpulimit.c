 ...
 * Version 2.1
 *
 * Updated by Henry Spanka - myVirtualserver <henry@myvirtualserver.de> for Proxmox VE
 */
...
int killrunninglimiter(pid)
{
   ...
   char search[4096];
   char killcommand[4096];
   sprintf(search, "/usr/sbin/qmcpulimit-b-q-z-p%d", pid);
         ...
         *killcommand = 0;
         if (res == 1)
         {
            ...
            *taskName = 0;
            ...
            while(getdelim(&arguments, &size, 0, cmdline) != -1) {
                strcat(taskName, arguments);
            }
               // is it the process we care about?
               if (strstr(taskName, search) != 0)
               {
                  sprintf(killcommand, "kill -9 %d", pid);
                  system(killcommand);
...
void print_usage(FILE *stream,int exit_code) {
        fprintf(stream, "CPUlimit version %1.1f-custom-1 - <henry@myvirtualserver.de>\n", VERSION);
